    ...
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div class="mt-6 bg-white shadow-sm rounded-lg divide-y">
            @foreach ($chirps as $chirp)
                @if (!Auth::user()->blockedUsers()->where('blocked_id', $chirp->user_id)->exists())
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                @if (!$chirp->user->is(auth()->user()))
                                    @if (Auth::user()->following()->where('followed_id', $chirp->user->id)->exists())
                                        <form action="{{ route('unfollow', $chirp->user) }}" method="POST">
                                            @csrf
                                            <button class="w-20 bg-red-600 text-white rounded-md p-2"
                                                type="submit">Unfollow</button>
                                        </form>
                                    @else
                                        <form action="{{ route('follow', $chirp->user) }}" method="POST">
                                            @csrf
                                ...
                                @else
                                    <x-dropdown>
                                        ...
                                        <x-slot name="content">
                                            <form method="POST" action="{{ route('block', $chirp->user_id) }}">
                                                @csrf
                                                <x-dropdown-link :href="route('block', $chirp->user)"
                                                    onclick="event.preventDefault(); this.closest('form').submit();">
                                                    {{ __('Block user') }}
